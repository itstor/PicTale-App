...
class MapViewModel @Inject constructor(
    ...
    val storiesParams = MutableLiveData<Pair<Int?, Int?>>()
    val stories = storiesParams.switchMap { (page, limit) ->
        getAllStories(page, limit)
    }

    private fun getAllStories(page: Int? = null, limit: Int? = null) = liveData {
        emit(Result.loading(null))
        try {
            val token = authRepo.getAuthToken().first() ?: ""

            val response = storyRepo.getAllStories(token, page, limit, 1)
